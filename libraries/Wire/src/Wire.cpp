...
TwoWire::TwoWire(uint32_t base) : m_base(base)
{
...
int TwoWire::requestFrom(int address, int quantity, int iaddress, int isize, int sendStop)
{
    if (isize > 0)
    {
...
int TwoWire::endTransmission(int sendStop)
{
    ...
    uint8_t ret = i2c_write(m_base, txAddress, txBuffer, txBufferLength);
...
// slave tx event callback
// or after beginTransmission(address)
size_t TwoWire::write(const void *data, size_t quantity)
{
    const uint8_t *ptr_byte = (const uint8_t *)data;
//    if (transmitting)
    {
        // in master transmitter mode
